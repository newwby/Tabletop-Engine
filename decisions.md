...
- [Agent System Design](#agent-system-design)
  ...
  - [Initial agent layers and introduction order](#initial-agent-layers-and-introduction-order)
  - [Anti-overlap governance checks](#anti-overlap-governance-checks)
  - [Scope creep and overengineering controls](#scope-creep-and-overengineering-controls)
  - [IP conflict avoidance in agent responsibilities](#ip-conflict-avoidance-in-agent-responsibilities)
  - [Decision logging expectations](#decision-logging-expectations)
...
### Initial agent layers and introduction order
**Decision**: Use a four-layer setup baseline and introduce agents in a fixed order.

**Layer Model**:
1. Orchestration layer (Orchestrator)
2. Generation layer (Lore Builder)
3. Validation layer (Continuity Auditor)
4. Audit and progress layer (Historian)
...
**Rationale**: Enforces clear ownership early, adds quality gates before scale, and preserves traceable progress/accounting.

### Anti-overlap governance checks
**Decision**: Require explicit anti-overlap checks for any agent creation or role change.

**Required Checks**:
- Single-owner responsibility mapping
- Boundary non-goal definition
- Output contract uniqueness
- Delegation-first routing when ownership already exists
- Escalation path for ambiguous ownership

**Rationale**: Prevents duplicated authority, conflicting outputs, and deadlocks.

### Scope creep and overengineering controls
**Decision**: Enforce strict MVP-first controls for agent/layer expansion.

**Implementation**:
- Do not add net-new agents/layers without a documented blocked workflow
- Prefer role refinement over introducing additional agents
- Time-box exploration and require explicit accept/defer/reject outcomes

**Rationale**: Protects delivery velocity and maintainability.

### IP conflict avoidance in agent responsibilities
**Decision**: Agent responsibilities must include explicit IP safety rules.

**Implementation**:
- Avoid reproducing copyrighted/proprietary settings or text without explicit rights
- Prefer original content for generated worldbuilding
- Escalate rights-unclear requests for human review

**Rationale**: Reduces legal and ethical risk in generated campaign content.

### Decision logging expectations
**Decision**: Exploration and design work must end with explicit decision-log outcomes.

**Implementation**:
- Record accept/defer/reject decisions and rationale
- Link deferred decisions to a review trigger
- Avoid re-opening settled decisions without new evidence or changed constraints

**Rationale**: Prevents endless exploration loops and preserves forward progress.
